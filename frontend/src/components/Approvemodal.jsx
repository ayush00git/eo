...
import { Building2, Calendar, Clock, Users, AlertTriangle, Check } from "lucide-react";
...
const ApproveModal = ({ isOpen, onClose, selectedEvent, onSubmit, venues, approvedBookings }) => {
    ...
    return (
        <ModalShell isOpen={isOpen} onClose={close} title="Approve booking" loading={loading}>
            ...
            <button
                className="mt-4 flex w-full items-center justify-center gap-1.5 rounded-md bg-emerald-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-emerald-700"
                onClick={handleSubmit}
            >
                <Check className="size-4" />
                Approve with {adminMessage ? "attached message" : "auto message"}
            </button>
        </ModalShell>
    );
};
